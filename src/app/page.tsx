  ...
  return (
    <>
      <section className="heroSection sectionShell" aria-labelledby="hero-title">
        ...
        <div className="heroGrid">
          ...
          <div className="livingFrame" aria-label="Living Film Frame">
            ...
            <div className="frameCaption"><strong>{data.featuredFilm.title}</strong><span>{data.featuredFilm.thesisQuestion}</span></div>
          </div>
        </div>
      </section>
        ...
        <div className="lensGrid">
          {data.lenses.map((lens, index) => (
            <article className="lensItem" key={lens.key}>
              <span className="lensNumber">0{index + 1}</span><h3>{lens.label}</h3><p>{lens.prompt}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="sectionShell sectionRule splitSection" aria-labelledby="story-title">
        <div><div className="sectionIndex">03 / STORY</div><h2 id="story-title">Understand the story before judging it.</h2><p className="sectionIntro">Plot structure, causality and turning points remain separate from moral scoring.</p></div>
        <ol className="storyTrack">
          {data.storyBeats.map((beat, index) => (
            <li key={beat.label}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{beat.label}</strong><p>{beat.summary}</p></div></li>
          ))}
        </ol>
      </section>
        ...
        <div className="characterGrid">
          {data.characters.map((character) => (
            <article className="characterCard" key={character.id}>
              <div className="portraitPlaceholder" aria-hidden="true" /><h3>{character.name}</h3>
              <dl><div><dt>Wants</dt><dd>{character.wants}</dd></div><div><dt>Fears</dt><dd>{character.fears}</dd></div><div><dt>Contradiction</dt><dd>{character.contradiction}</dd></div></dl>
            </article>
          ))}
        </div>
        <div className="relationshipPanel" aria-labelledby="relationship-title">
          <div><span className="microLabel">RELATIONSHIP OBSERVATORY</span><h3 id="relationship-title">{data.relationship.label}</h3><p>{data.relationship.summary}</p></div>
          <ol className="relationshipTrace">
            {data.relationship.events.map((event) => (
              <li className={`traceEvent trace-${event.tone}`} key={event.id}><span className="traceDot" aria-hidden="true" /><div><strong>{event.label}</strong><p>{event.change}</p></div></li>
            ))}
          </ol>
        </div>
      </section>
      ...
      <section className="sectionShell sectionRule meaningSection" aria-labelledby="meaning-title">
        ...
        <div className="meaningGrid"><div><span>Theme</span><strong>{data.meaning.theme}</strong></div><div><span>Apparent claim</span><strong>{data.meaning.apparentClaim}</strong></div><div><span>Counterevidence</span><strong>{data.meaning.counterevidence}</strong></div><div><span>Confidence</span><strong>{data.meaning.confidence}</strong></div></div>
      </section>

      <section className="sectionShell sectionRule" aria-labelledby="permission-title">
        <div className="sectionIndex">07 / NARRATIVE PERMISSION</div><h2 id="permission-title">What does the film make costly, questionable, normal or rewarding?</h2>
        <div className="permissionField">{data.permissions.map((permission) => <article key={permission.subject}><span className="permissionState">{permission.state}</span><h3>{permission.subject}</h3><p>{permission.rationale}</p></article>)}</div>
      </section>

      <section className="sectionShell sectionRule splitSection" aria-labelledby="craft-title">
        <div><div className="sectionIndex">08 / FORM SHAPES SYMPATHY</div><h2 id="craft-title">What does the film make us feel before it asks us to think?</h2></div>
