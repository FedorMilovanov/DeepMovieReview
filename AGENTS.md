...
- Real-film packages must declare `FilmPackage.ingest.edition`. `TARGET_ONLY` normally forbids canonical scenes/evidence and all analytical modules except `sources-method`. The only pre-lock exception is an explicit `FilmPackage.research.state = "SECONDARY_SOURCES"` tier: it may carry **DRAFT-only** scene estimates, non-`film-edition` secondary-source evidence and analytical working modules, but it is structurally unpublishable and none of that material is canonical. On master lock, research-tier material must be re-verified against the exact viewing master; only `LOCKED` may own canonical scenes/evidence, and every canonical evidence record must reference that locked `film-edition` source. Never use the research tier to simulate master verification.
