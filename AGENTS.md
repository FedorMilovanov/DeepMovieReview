...
- Real-film packages must declare `FilmPackage.ingest.edition`. `TARGET_ONLY` forbids canonical evidence; only `LOCKED` may own real-film evidence, and every such evidence record must reference that locked `film-edition` source. Never bypass this gate to accelerate Film 001 authoring.
