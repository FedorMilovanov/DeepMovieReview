...
- machine-readable real-film edition state machine: `TARGET_ONLY` selects a target, `MASTER_IDENTIFIED` records an exact measured viewing master while canonical promotion stays fail-closed, and only `LOCKED` permits canonical scene/evidence chains; the explicit `SECONDARY_SOURCES` research tier may coexist with either pre-lock state and may carry only provisional DRAFT scene estimates, secondary-source evidence and unpublishable working modules; `LOCKED` canonical evidence must cite the exact locked `film-edition` source;
- exact-master structural segmentation is modeled separately from editorial scenes: optional `ingest.masterSegmentation` may record complete embedded-chapter ranges for provenance/navigation once a master is measured, but those segments do not become `FilmPackage.scenes` automatically; editorial scenes still require independent master review and verification;
- real-film source metadata integrity: methodology version, editorial revision and human-readable analyzed edition are required already during draft authoring;
- canonical edition-bound `scenes[]` with numeric time ranges and verification state; outside `SECONDARY_SOURCES`, real-film evidence scene references must resolve to VERIFIED scenes, while research-tier references may point only to DRAFT scene estimates until lock-time re-verification; numeric evidence timestamps are range-checked in either tier;
- spoiler monotonicity from scene → evidence/autopsy: derived scene content may stay at the same level or become more restricted, never less restricted than its referenced scene, whether that scene is a research-tier DRAFT estimate or a canonical VERIFIED scene;
- evidence locator integrity: every canonical real-film evidence record — including LOCKED draft work — must include a VERIFIED scene ID plus numeric timestamp in the locked master; research-tier evidence instead remains explicitly provisional, references DRAFT scene estimates plus secondary sources, and must be re-anchored at master lock; published packages may contain only VERIFIED scenes;
...
- evidence-backed Story structure: real-film Story summary and every plot beat carry ClaimSupport; in the canonical LOCKED path that support resolves to VERIFIED scene/timestamp evidence, while `SECONDARY_SOURCES` support remains provisional until lock-time re-verification;
- evidence-backed Decision / Knowledge Fog: options, knowledge facts, pressures and duties/goods each carry ClaimSupport instead of relying only on the final editorial judgment; research-tier support stays provisional, while LOCKED support follows the canonical evidence chain;
