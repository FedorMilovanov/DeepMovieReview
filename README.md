...
- machine-readable real-film edition locking: `TARGET_ONLY` normally permits only `sources-method` and forbids canonical scenes/evidence; the explicit `SECONDARY_SOURCES` research tier is the sole pre-lock exception and may carry only provisional DRAFT scene estimates, secondary-source evidence and unpublishable working modules; `LOCKED` canonical evidence must cite the exact locked `film-edition` source;
